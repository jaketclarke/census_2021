...
unpivot_merge_filepath = f'{dir}{os.sep}summary{os.sep}2021Census_VIC_SED_2022_unpivoted_ranks_merged.csv'
...
if not os.path.isfile(unpivot_merge_filepath):
    ...
    for file in get_files_in_directory(dir, suffix=".csv"):
        if 'unpivoted.csv' in file and 'ranked' in file:
            # get data
    ...
    # reshape and order
    df_unpivot_all = df[['table','census_variable','district','rank']].sort_values(['table','census_variable','district'])
    df_unpivot_all.to_csv(unpivot_merge_filepath, index=False, na_rep="Null")

pivot_questions_filepath = f'{dir}{os.sep}summary{os.sep}2021Census_VIC_SED_2022_ranks_wide_combined.csv'
if not os.path.isfile(pivot_questions_filepath):
    df = pd.read_csv(unpivot_merge_filepath, na_values=['Null','NaN','nan','Nan'])
    pivot = df.pivot(index='district', columns=['table', 'census_variable'], values='rank')
    pivot.to_csv( pivot_questions_filepath, na_rep='Null')

pivot_districts_filepath = f'{dir}{os.sep}summary{os.sep}2021Census_VIC_SED_2022_ranks_long.csv'
if not os.path.isfile(pivot_districts_filepath):
    df = pd.read_csv(unpivot_merge_filepath, na_values=['Null','NaN','nan','Nan'])
    df = df[~df['census_variable'].str.endswith('_pc')]
    pivot = df.pivot(index=['table', 'census_variable'], columns='district', values='rank')
    pivot.to_csv(pivot_districts_filepath, na_rep='Null')
